...
public interface MeTodoRepository extends TodoRepository {
    ...
    @Query(value = """
            ...
            LEFT JOIN image e ON a.image_id = e.id 
            WHERE c.id = :idPeriod
            AND a.todo_list_id = :idTodoList 
            ORDER BY a.index_todo
            """, nativeQuery = true)
    List<MeTodoResponse> getToDoByPeriodAndIdTodoList(@Param("idPeriod") String idPeriod, @Param("idTodoList") String idTodoList);
    ...
    @Query(value = """
                UPDATE to_do t
                 JOIN (
                   SELECT g.id, ROW_NUMBER() OVER (PARTITION BY g.todo_list_id ORDER BY COALESCE(g.priority_level, 9999999) ASC) AS new_index
                   FROM to_do g
