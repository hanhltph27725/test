window.MemberPeriodProjectController = function (
  ...
  MeDetailProjectService.fetchProject(idProject).then(function () {
    ...
    document.title = "Quản lý giai đoạn | " + $scope.detailProject.name;
  });

  function loadDataListPeriod() {
    MeGetAllPeriod.fetchPeriods(idProject).then(function () {
      $scope.listPeriod = MeGetAllPeriod.getPeriods();
      $scope.listPeriod.forEach((item) => {
        item.convertDateStartTime = ConvertLongToDateString.convertLongToDate(
          item.startTime
        );
        item.convertDateEndTime = ConvertLongToDateString.convertLongToDate(
          item.endTime
        );
      });
  ...
  $scope.actionDetailPeriod = function (event, id) {
    ...
    MeDetailPeriod.fetchPeriod(id).then(function () {
      ...
      $scope.detailPeriod.statusPeriodStr =
        $scope.detailPeriod.statusPeriod == "CHUA_DIEN_RA"
          ? "Chưa diễn ra"
          : $scope.detailPeriod.statusPeriod == "DA_DIEN_RA"
          ? "Đã diễn ra"
          : "Đang diễn ra";
    });
  };
  ...
  $scope.actionAdd = function () {
    ...
    if ($scope.ngayBatDauAdd === "" || $scope.ngayBatDauAdd == null) {
      $scope.errorNgayBatDauAdd = "Ngày bắt đầu không được để trống";
      ++check;
    } else if ($scope.ngayKetThucAdd === "" || $scope.ngayKetThucAdd == null) {
    ...
    if ($scope.ngayKetThucAdd === "" || $scope.ngayKetThucAdd == null) {
      $scope.errorNgayKetThucAdd = "Ngày kết thúc không được để trống";
      ++check;
    } else {
      $scope.errorNgayKetThucAdd = "";
